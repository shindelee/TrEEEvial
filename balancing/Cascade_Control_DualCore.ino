...
double velocity_kP = 0.5;  // PID gains for velocity control
double velocity_kI = 0.01;
double velocity_kD = 0.3;
int LIMIT = 500;

double maxAcceleration = 100.0;  // Maximum acceleration in steps/s^2
...
void loop() {
  ...
  //  CurrAngle = mpu.getAngleY();
  //  Serial.println(CurrAngle);
  //  timer = millis();
  //}

  // Angle PID control
  angleSetpoint = 0;  // Desired angle (set as per your requirements)
  angleInput = CurrAngle;
  ...
  velocitySetpoint = angleOutput;  // Set the target velocity as the output of the angle PID
  velocityInput = currentVelocity;

  velocityPid.Compute();

  // Update the target velocity based on the output of the velocity PID
  targetVelocity = abs(velocityOutput);
  ...
  stepperSpeed = currentVelocity;

  // Dead band angle
  if (CurrAngle < 3 && CurrAngle > -3) {
    stepperSpeed = 0;
    currentVelocity = 0;
  }
