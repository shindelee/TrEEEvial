...
float va,vb,vref,iL,dutyref,current_mA,current_InputVoltage, current_InputCurrent, current_Power, power, vlast; // Measurement Variables
...
float gridlimit = 12; // LIMIT VOLTAGE for Grid
...
void setup() {
  ...
  analogReference(EXTERNAL); // We are using an external analog reference for the ADC
...
void loop() {
  if(loopTrigger) { // This loop is triggered, it won't run unless there is an interrupt
    ...
    // other parts of the code are identical to the lab code

    if (Boost_mode){
      if (CL_mode) { //Closed Loop Boost
        current_limit = 2.4; //
        current_InputVoltage = vb;
        current_InputCurrent = iL;
        current_Power = current_InputVoltage*current_InputCurrent;
        // power is the previous power
        ...
        else { // when the current power is less than the previous power
            if (current_InputVoltage > vlast) { 
              //closed_loop += 0.01;
              closed_loop -= 0.01;
            }
        ...
        //to set upperbound on the grid voltage (va is output voltage)
        if (va > gridlimit){
        ...
        power = current_Power;
        closed_loop = saturation(closed_loop, 0.99, 0.1);  //duty_cycle saturation    
        vlast = current_InputVoltage;
...
void sampling(){
  ...
  // We want to treat it as an input current in the Boost, so it's also inverted
