...
while True:
    ...
    iref = vPID.controller(vout, vref) # produce a control signal that has a value between 0 and 0.5
    ...
    # PID for stability, conditional logic for power tuning

    if vin < 6.25 and vin >= 5.75:
        duty_ref += 5
        pwm.freq(freq - 12500)
    elif vin < 5.75 and vin >= 5:
        duty_ref += 7.5
        pwm.freq(freq - 17500)
    elif vin <5 and vin >= 4.5:
        duty_ref += 10
        pwm.freq(freq-22500)
    elif vin < 4.5 and vin >= 4:
        duty_ref += 17.5
        pwm.freq(freq-37500)
